...
class BasicBlock(layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        x = self.conv1(inputs)
        x = self.bn1(x, training=training)  # bn层只需要在训练集上使用，不需要在测试集上
        x = self.relu(x)

        x = self.conv2(x)
        x = self.add([identity, inputs])
        x = self.relu(x)
...
class Bottleneck(layers.Layer):
    expansion = 4 # 残差结构中卷积核变化的系数

    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        super(Bottleneck, self).__init__(**kwargs)
        self.conv1 = layers.Conv2D(out_channel, kernel_size=1, name="conv1", use_bias=False)
        ...
        # -------------------------------------------------
        self.conv3 = layers.Conv2D(out_channel * self.expansion, kernel_size=1,
...
def _resnet(block, block_num, im_width=224, im_height=224, num_classes=1000, include_top=True):
    # block是选择BasicBlock（18、34）还是Bottleneck（50、101、152）
    # block_num是选择列表类型,conv n.x对应的残差块个数
    # include_top针对resnet最后的平均池化下采样喝全连接层
